import argparse

from .preprocessing_families import *
from ..logger import log_run

def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Preprocessing pipeline runner")
    p.add_argument(
        "--run",
        choices=["all", "all_families_data_summary", "filter_min_family_size", "preprocessing_summary_stats"],
        default="all",
        help="Which step to run",
    )
    p.add_argument(
        "--min-family-size",
        type=int,
        default=50,
        help="Minimum family size for filtering (used in 'filter_min_family_size' / 'all')",
    )
    return p.parse_args()


def main() -> None:
    args = parse_args()

    all_families_csv: Path = INPUT_DATA_DIR / ALL_FAMILIES_DATA_SUMMARY_FILE
    families_for_analysis_csv: Path = INPUT_DATA_DIR / "families_for_analysis.csv"
    summary_txt: Path = INPUT_DATA_DIR / "families_for_analysis_summary.txt"

    if args.run in ("all", "all_families_data_summary"):
        total_fams, skipped_fams, kept_fams = families_data_summary_to_csv(
            database_chr_count_input=DATABASE_INPUT_DIR,
            preprocessing_dir=INPUT_DATA_DIR,
        )
    else:
        total_fams = skipped_fams = kept_fams = None

    if args.run in ("all", "filter_min_family_size"):
        total_filter, excluded_filter, kept_filter = filter_families_by_minimum_num_of_species(
            families_data_summary=all_families_csv,
            min_family_size=args.min_family_size,
            preprocessing_dir=INPUT_DATA_DIR,
        )
    else:
        total_filter = excluded_filter = kept_filter = None

    if args.run in ("all", "preprocessing_summary_stats"):
        preprocessing_summary(
            families_for_analysis_file=families_for_analysis_csv,
            preprocessing_dir=INPUT_DATA_DIR,
            columns=PREPROCESSING_COLUMNS,
        )

    outputs: list[str] = []
    params: dict[str, object] = {"run": args.run}
    notes_parts: list[str] = []

    if args.run in ("all", "all_families_data_summary"):
        outputs.append(str(all_families_csv))
        params["database_chr_count_input"] = str(DATABASE_INPUT_DIR)
        params["preprocessing_dir"] = str(INPUT_DATA_DIR)

        notes_parts.append(
            f"Iterated over {total_fams} family folders; "
            f"skipped {skipped_fams} missing {CHROM_COUNTS_FILE_NAME}; "
            f"kept {kept_fams}."
        )

    if args.run in ("all", "filter_min_family_size"):
        outputs.append(str(families_for_analysis_csv))
        outputs.append(str(INPUT_DATA_DIR / f"excluded_families_min_size_{args.min_family_size}.txt"))

        params["min_family_size"] = args.min_family_size
        params["families_data_summary"] = str(all_families_csv)
        params["preprocessing_dir"] = str(INPUT_DATA_DIR)

        if excluded_filter is not None:
            notes_parts.append(
                f"Filter(min_family_size={args.min_family_size}): "
                f"excluded {excluded_filter}/{total_filter}; kept {kept_filter}."
            )

    if args.run in ("all", "preprocessing_summary_stats"):
        outputs.append(str(summary_txt))
        params["families_for_analysis_file"] = str(families_for_analysis_csv)
        params["columns"] = PREPROCESSING_COLUMNS
        params["preprocessing_dir"] = str(INPUT_DATA_DIR)

    notes = " ".join(notes_parts) if notes_parts else None

    log_run(
        step="preprocessing",
        script=Path(__file__),
        params=params,
        outputs=outputs,
        description=(
            "Preprocessing pipeline (controlled by --run): "
            "all families data summary csv, filter, and/or preprocessing summary stats."
        ),
        notes=notes,
    )


if __name__ == "__main__":
    main()
